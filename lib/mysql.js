...
var debug = require('debug')('app:mysql:');
...
/* Inicio de sesión
 * La base de datos llama a un procedimiento que dara como resultado
 * un query con un valor booleano. Si el query esta vacio se genera un
 * error 404. En caso contrario se devuelve el resultado booleano.
 */
driver.login = ({ user, pass }) => new Promise( (resolve, reject ) =>
connection.query("CALL login(?,?);", [user, pass],
(err, result) => {
	debug(result);
	if(err) reject(err);
	else if( result[0].length == 0 ) reject({status: 404});
	else resolve( result[0][0].log );
}));
